...
def main():
    ...
    DELETE_WHEN_DONE = (
        os.getenv("RC_DELETE_POSTS_AFTER_RUN", "False").lower() == "true"
    )  # noqa E501
    ...
    Command.add_command(
        Command(hashtag="test", function=lambda status: "test", help_arguments={})
    )
...
#         logger.debug(f"Got update: {status['content']}")  # noqa E501
...
#                 status=notification["status"], always_mention=ALWAYS_MENTION
#             )
